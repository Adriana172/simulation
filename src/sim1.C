...
vector<Road *>
create_roads(const GeoOctuplet &geometry, bool uvrflag, int m_xthr, int m_uvthr, string chamber, bool trapflag)
{
...
tuple<int, vector<slope_t>> finder(vector<Hit *> hits, int mu_firstbc, vector<Road *> roads, bool saveHits,
                                   bool ideal_vmm, bool ideal_addc, bool ideal_tp, int evt)
{
   ...
   for (int bc = bc_start; bc < bc_end; bc++) {
      // increments the bc and repeats the steps
      hits_now.clear();
      ...
      // sends the hits with the first BC
      ...
      for (int ib = 0; ib < NPLANES; ib++) {
         for (int j = 0; j < n_vmm; j++) {
            ...
            for (unsigned int k = 0; k < hits_now.size(); k++) {
               if (hits_now[k]->MMFE8Index() != ib) // only registers vmm for the first plane
                  continue;
               if (hits_now[k]->VMM() == j) {
      ...
      for (unsigned int i = 0; i < roads.size(); i++) {
         ...
         if (roads[i]->Coincidence(bc_wind) && bc >= (mu_firstbc - 1)) {
            ...
            if (db) {
               ...
               cout << "---------------------------" << endl;
               /////////////PRINTS ALL THE HITS THAT I NEED !!!!!!!!!!!!!!!!!!!
               //// I try to BC 
               for (unsigned int k = 0; k < roads[i]->Hits().size(); k++) {
                  printf("Hit (board, BC, strip): (%d,%d,%4.4f)", roads[i]->Hits()[k].MMFE8Index(),
                         roads[i]->Hits()[k].Age(), roads[i]->Hits()[k].Channel());
                  cout << endl;
               }
            }
            ntrigs++;
            ...
            double xcenter = 0;
            double ycenter = 0;
            std::tie(xcenter, ycenter) =
               roads[i]->Center(XROAD, NSTRIPS_UP_XX, NSTRIPS_DN_XX, NSTRIPS_UP_UV, NSTRIPS_DN_UV);
            ...
            m_slope.yavg = -B *
                              (roads[i]->AvgXofU() - roads[i]->AvgXofV() +
                               (roads[i]->AvgZofV() - roads[i]->AvgZofU()) * roads[i]->Mxl()) /
                              2 +
                           yhigh;
...
void progress(double time_diff, int nprocessed, int ntotal)
{
   ...
   std::cout << "\r > " << nprocessed << " / " << ntotal << " | " << std::fixed
             << 100 * (double)(nprocessed) / (double)(ntotal) << "%"
             << " | " << std::fixed << rate << "Hz"
             << " | " << std::fixed << time_diff / 60 << "m elapsed"
             << " | " << std::fixed << (double)(ntotal - nprocessed) / (rate * 60) << "m remaining" << std::flush;
}
...
void plttrk(vector<Hit> hits, bool xflag, TString title, int ntrig, TFile *file)
{
   ...
   Double_t zpos[8] = {0., 11.2, 32.4, 43.6, 113.6, 124.8, 146.0, 157.2};
...
int main(int argc, char *argv[])
{
   ...
   for (int i = 0; i < nevents; i++) {
      ...
      for (int j = 0; j < NPLANES; j++) {
         if (oct_hitmask[j] == 1) {
            ...
            // sanity check
            if (j < 2 || j > 5)
               hists["h_xres_strip"]->Fill(
                  xpos[j] - (GEOMETRY->Get(j).Origin().X() + GEOMETRY->Get(j).LocalXatYbegin(strip_smear)));
      ...
      for (unsigned int ihit = 0; ihit < all_hits.size(); ihit++) {
         ...
         if (all_hits[ihit]->IsNoise() && (ib < 2 || ib > 5))
            hists["h_dx"]->Fill(GEOMETRY->Get(ib).LocalXatYend(all_hits[ihit]->Channel()) +
                                GEOMETRY->Get(ib).Origin().X() - xmuon);
      }
      ...
      std::tie(ntrigroads, m_slopes) =
         finder(all_hits, smallest_bc, m_roads, (pltflag || write_tree), ideal_vmm, ideal_addc, ideal_tp, i);
   ...
   cout << "n=1: " << nevent_uvbkg1 << " | n=2: " << nevent_uvbkg2 << " | n=3: " << nevent_uvbkg3
        << " | n=4: " << nevent_uvbkg4 << endl;
   ...
   mylog << "n=1: " << nevent_uvbkg1 << " | n=2: " << nevent_uvbkg2 << " | n=3: " << nevent_uvbkg3
         << " | n=4: " << nevent_uvbkg4 << "\n";
