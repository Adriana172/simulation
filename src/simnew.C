...
vector<Hit *> finder(vector<Hit *> hits, int mu_firstbc, bool ideal_vmm, bool ideal_addc, bool ideal_tp)
{
...
#define MAX_SIZE nevents
   ...
   for (int i = 0; i < nevents; i++) {
      ...
      cout << "HITS VECTOR IS: " << hits[1] << endl;

      if (true) {
         cout << "N muonhits: " << hits.size() << endl;
         printf("%d SMALL HITS ", hits);
      ...
      for (unsigned int ihit = 0; ihit < all_hits.size(); ihit++) {
         ...
         //       printf("%f\n",
         //              GEOMETRY->Get(ib).LocalXatYend(all_hits[ihit]->Channel()) + GEOMETRY->Get(ib).Origin().X() -
         //              xmuon);
      }
      ...
      vector<Hit *> finder1;
      finder1 = finder(all_hits, smallest_bc, ideal_vmm, ideal_addc, ideal_tp);
      cout << "Found1 " << finder1[1] << " and found2 " << finder1[2] << endl;
   }
